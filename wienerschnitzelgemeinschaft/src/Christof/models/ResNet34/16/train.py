...
MODEL_PATH = 'Christof/models/ResNet34/16/'
exp_suffix = '_base'
...
for thresh in np.arange(0.1, 0.701, 0.01):
    ...
    score = f1_score(y_true, (preds > thresh).astype(int),average='macro')
    if score > best_score:
        best_thresh = thresh
        best_score = score

print("Val F1 Score at {:.2}: {:.4f}".format(best_thresh,best_score))