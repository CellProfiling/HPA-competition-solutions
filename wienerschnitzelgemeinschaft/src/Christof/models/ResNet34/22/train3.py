...
MODEL_PATH = 'Christof/models/ResNet34/22/'
exp_suffix = '_3'
...
def create_model(input_shape, n_out):
    ...
    #x = Dropout(0.5)(x)
    x = Conv2D(28, kernel_size=(1, 1), activation='sigmoid')(x)
...
model = create_model(
    ...
    n_out=28)
#model.load_weights('Christof/models/ResNet34/21/model_loss_base.h5')
...
model.load_weights(MODEL_PATH + 'model_loss_2.h5')
model.fit_generator(
    train_generator,
...
submit['Predicted'] = str_predict_label
# np.save('draw_predict_InceptionV3.npy', score_predict)
submit.to_csv(MODEL_PATH + 'submission_loss{}_lb_dist_adjusted_16tta.csv'.format(exp_suffix), index=False)
