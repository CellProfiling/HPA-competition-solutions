...
MODEL_PATH = 'models/ResNet34/tests/10/'
SIZE = 256

# Load dataset info
path_to_train = 'assets/train_256/'
data = pd.read_csv('assets/train.csv').sample(frac=0.5)
...
for train_indexes, valid_indexes in kf:
    ...
    f1_macro = F1Metric(validation_generator,steps=len(valid_indexes)//batch_size,batch_size=batch_size,num_classes=28)
    # f1_macro resuls in an additional metric "val_f1_all" in history which can be used to checkpoint
    checkpoint = ModelCheckpoint(MODEL_PATH + 'model_{}.h5'.format(fold_id), monitor='val_f1_all', verbose=1,
                                 save_best_only=True, mode='max', save_weights_only=True)
    tensorboard = TensorBoard(MODEL_PATH + 'logs{}/'.format(fold_id))
    callbacks_list = [f1_macro,checkpoint, tensorboard]
