...
data = pd.read_csv('Christof/assets/train.csv').sample(frac=0.3, random_state=17)
...
normal_aug = A.Compose([A.OneOf([A.Rotate((-180,180)),
                        ...
                        A.Flip(p=0.75),
                        A.ShiftScaleRotate(shift_limit=0.1, scale_limit=0, rotate_limit=0, p=.5),
                        A.PadIfNeeded(min_width=SIZE, min_height=SIZE)
                        ])
