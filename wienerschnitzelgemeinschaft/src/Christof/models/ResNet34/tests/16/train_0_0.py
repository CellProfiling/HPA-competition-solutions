...
MODEL_PATH = 'Christof/models/ResNet34/tests/16/'
exp_suffix = '0_0'
...
#def get_fold_ids(fold_id,data, shuff = True):
#    val_ids = data[data['fold'] == fold_id].index.values
#    train_ids = data[data['fold'] != fold_id].index.values
#    if shuff:
#        shuffle(val_ids)
#        shuffle(train_ids)
...
train_generator = data_generator.create_train(train_dataset_info[train_indexes],
                                              batch_size, (SIZE, SIZE, 3), augument=True, heavy_augment_rares=True, oversample_factor=0)
...
#                      mode="min",
#                      patience=6)
f1_metric = F1Metric(validation_generator,len(valid_indexes)//batch_size,batch_size,28)
...
valid_df = data.iloc[valid_indexes]
