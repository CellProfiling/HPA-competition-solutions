...
MODEL_PATH = 'Christof/models/ResNet34/tests/20/'
exp_suffix = '0'
    ...
    def create_train(dataset_info, batch_size, shape, augument=True, heavy_augment_rares=True, oversample_factor = 0):
        ...
        while True:
            ...
            for start in range(0, len(dataset_info), batch_size):
                ...
                batch_nlabels = np.zeros((len(X_train_batch), 1))
                for i in range(len(X_train_batch)):
                    ...
                    batch_nlabels[i] = (len(X_train_batch[i]['labels'])-1)/4
                yield np.array(batch_images, np.float32), [batch_labels,batch_nlabels]
...
def create_model(input_shape, n_out):
    ...
    model = Model(input_tensor, outputs = [output,out2])
...
#                      mode="min",
#                      patience=6)
f1_metric = F1MetricNumber(validation_generator,len(valid_indexes)//1,1,28)
...
model.compile(
    loss=['binary_crossentropy','binary_crossentropy'],loss_weights=[1,0.01],
...
model.compile(
    loss=['binary_crossentropy','binary_crossentropy'],loss_weights=[1,0.1],
    optimizer=Adam(1e-03),
    metrics=['acc',f1])
model.fit_generator(
    train_generator,
