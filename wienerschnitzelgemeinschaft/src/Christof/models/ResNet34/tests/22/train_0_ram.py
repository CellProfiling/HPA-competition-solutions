...
heavy_aug = A.Compose([A.HorizontalFlip(p=0.5),
                        A.VerticalFlip(p=0.5),
                        A.Rotate((-180,180))])
train_dataset_info = []
...
class data_generator:
    ...
    @staticmethod
    def create_train(dataset_info, batch_size, shape, augumentation=None, oversample_factor = 0, into_ram = True):
        assert shape[2] == 3
        ...
        if into_ram:
            #images = np.zeros((len(dataset_info),shape[0],shape[1],shape[2]))
            for i in tqdm(range(len(dataset_info))):
                dataset_info[i]['image'] = data_generator.load_image(dataset_info[i]['path'])
                #images[i] = data_generator.load_image(dataset_info[i]['path'], shape)
            ...
            for start in range(0, len(dataset_info), batch_size):
                ...
                for i in range(len(X_train_batch)):
                    ...
                    if augumentation is not None:
                        image = data_generator.augment(augumentation,image)
...
train_generator = data_generator.create_train(train_dataset_info[train_indexes],
                                              batch_size, (SIZE, SIZE, 3), augumentation = None,oversample_factor=0,)
validation_generator = data_generator.create_train(train_dataset_info[valid_indexes],
                                                   1, (SIZE, SIZE, 3), augumentation = None, oversample_factor=0)
...
early = EarlyStopping(monitor='val_f1_all',
                      mode='max',
                      patience=8)
f1_metric = F1Metric(validation_generator,len(valid_indexes)//1,1,28)
callbacks_list = [f1_metric, checkpoint, tensorboard, early]
...
train_generator = data_generator.create_train(train_dataset_info[train_indexes],
                                              batch_size * 2, (SIZE, SIZE, 3), augumentation = normal_aug,oversample_factor=0,)
#validation_generator = data_generator.create_train(train_dataset_info[valid_indexes],
#                                                   1, (SIZE, SIZE, 3), augumentation = None, oversample_factor=0)
model.fit_generator(
    train_generator,
...
train_generator = data_generator.create_train(train_dataset_info[train_indexes],
                                              batch_size * 4, (SIZE, SIZE, 3), augumentation = heavy_aug,oversample_factor=0,)
#validation_generator = data_generator.create_train(train_dataset_info[valid_indexes],
#                                                   1, (SIZE, SIZE, 3), augumentation = None, oversample_factor=0)
model.fit_generator(
    train_generator,
