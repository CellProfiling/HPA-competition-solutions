...
MODEL_PATH = 'models/ResNet34/tests/5/'
SIZE = 256
...
def create_model(input_shape, n_out):
    ...
    conv = Conv2D(3,(3,3),padding='same',activation='relu')(input_tensor)
    base_model = ResNet34(include_top=False,
                             weights=None,
                             input_shape=(SIZE, SIZE, 3),input_tensor=conv)
    ...
    for i, l in enumerate(base_model.layers[offset+1:]):
        l.set_weights(res_img.layers[i + offset].get_weights())
