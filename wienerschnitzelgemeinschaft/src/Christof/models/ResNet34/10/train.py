...
MODEL_PATH = 'Christof/models/ResNet34/10/'
exp_suffix = '_base'
...
submit.to_csv(MODEL_PATH + 'submission{}_lb_dist_adjusted_notta.csv'.format(exp_suffix), index=False)
...
for i, name in tqdm(enumerate(submit['Id'])):
    path = os.path.join('Christof/assets/test_rgb_256/', name)
    image = data_generator.load_image(path, (SIZE, SIZE, 3))
    images = [data_generator.augment(image) for _ in range(tta)]
