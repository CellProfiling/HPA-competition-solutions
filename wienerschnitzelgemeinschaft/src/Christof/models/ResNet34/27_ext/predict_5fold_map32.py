...
for fold_id in range(5):
    draw_predict[fold_id] = np.load(MODEL_PATH + f'pred_map32{fold_id}.npy')


draw_predict1 = np.mean(draw_predict,axis = 0)
np.save(MODEL_PATH + f'pred_5fold_32.npy',draw_predict1)
...
submit.to_csv(MODEL_PATH + 'submission_loss_5fold_map32_mean.csv', index=False)
