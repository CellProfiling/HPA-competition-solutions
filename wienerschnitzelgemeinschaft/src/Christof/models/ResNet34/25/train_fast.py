...
MODEL_PATH = 'Christof/models/ResNet34/25/'
exp_suffix = '_fast'
    ...
    def create_train(dataset_info, batch_size, shape, augument=None, oversample_factor = 0):
        ...
        while True:
            ...
            for start in range(0, len(dataset_info), batch_size):
                ...
                batch_label_maps64 = np.zeros((len(X_train_batch), 64, 64, 28))
                for i in range(len(X_train_batch)):
                    ...
                    gr64 = cv2.resize(img_g, (64, 64), interpolation=cv2.INTER_AREA)
                    gr64 = binary_dilation(gr64).astype(int)
                    for kk in X_train_batch[i]['labels']:
                        batch_label_maps32[i,:, :, kk] = gr
                        batch_label_maps64[i, :, :, kk] = gr64


                    batch_images.append(image)
                    batch_labels[i][X_train_batch[i]['labels']] = 1
                yield np.array(batch_images, np.float32), [batch_labels,batch_label_maps32,batch_label_maps64]
...
def encoder(backbone):
    ...
    c2 = backbone.get_layer('stage2_unit1_relu1').output  # 64
...
def create_model(input_shape, n_out):
    ...
    x, short_cuts = encoder(base_model)

    #x = Dropout(0.5)(x)
    x_map64 = Conv2D(28, kernel_size=(1, 1), activation='sigmoid', name='map64')(short_cuts[1])
    ...
    model = Model(input_tensor, [output,x_map32,x_map64])
...
validation_generator = data_generator.create_train(train_dataset_info[valid_indexes],
                                                   2* batch_size, (SIZE, SIZE, 3), augument=val_aug, oversample_factor=0)
...
#f1_field_metric = F1MetricField(validation_generator2,len(valid_indexes)//(4*batch_size),2*batch_size,28)
...
loss_funcs = {
    ...
    "map64": weighted_binary_crossentropy
}
loss_weights = {"pred": 1.0, "map32": 5.0,"map64": 2.5 }

metrics = { "pred":[binary_accuracy, brian_f1],
            "map32":[binary_accuracy],
            "map64": [binary_accuracy]}
...
model.fit_generator(
    ...
    validation_steps=np.ceil(float(len(valid_indexes)) / float(2*batch_size)),
    epochs=epochs,
    verbose=1,
...
for i, name in tqdm(enumerate(submit['Id'])):
    ...
    tta_predicts, tta_predicts_map32, tta_predicts_map64 = model.predict(np.array(images))
...
sub2 = pd.read_csv('Christof/models/ResNet34/24/' + 'submission_loss_fast_lb_dist_adjusted_8tta.csv')
f1_sub(sub_best,sub2)