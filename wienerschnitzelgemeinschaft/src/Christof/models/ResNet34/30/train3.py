...
MODEL_PATH = 'Christof/models/ResNet34/30/'
exp_suffix = '_3'
    ...
    def create_train(dataset_info, batch_size, shape, augument=None, weighted_sample = True):
        ...
        while True:
            ...
            for start in range(0, len(dataset_info), batch_size):
                ...
                batch_label_maps32 = np.zeros((len(X_train_batch), 16, 16, 28))
                for i in range(len(X_train_batch)):
                    ...
                    gr = cv2.resize(img_g, (16, 16), interpolation=cv2.INTER_AREA)
...
model.save_weights(MODEL_PATH + f'model_loss{exp_suffix}.h5')
...
np.save(MODEL_PATH + f'pred_map32{fold_id}.npy',draw_predict_map32)
...
dp = (3*draw_predict + draw_predict_map32) / 4
pred = dp.copy()
thresholds = np.linspace(0.95, 0.05, 101)
