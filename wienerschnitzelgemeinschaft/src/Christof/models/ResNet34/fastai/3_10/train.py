...
from Christof.static import name_label_dict, channel_avr, channel_std, lb_prob
...
MODEL_PATH = 'Christof/models/ResNet34/fastai/3_10/'
...
def get_fold(fold_id,folds):
    tr_n = list({fn for fn in folds if folds[fn]!=fold_id})
    val_n = list({fn for fn in folds if folds[fn] == fold_id})
    return tr_n, val_n
...
for fold_id in range(10):
    sz = 256 #image size
    bs = 64  #batch size
    tr_n, val_n = get_fold(fold_id,folds)
    md = get_data(sz,bs,tr_n, val_n)
    learner = ConvLearner.pretrained(arch, md, ps=0.5) #dropout 50%
    learner.models_path = MODEL_PATH
    ...
    # save model
    learner.save(f'ResNet34_256_{fold_id}')
    ...
    preds_dict[str(fold_id)] = pred
    preds_t_dict[str(fold_id)] = pred_t

import pickle

with open('preds_dict','wb') as f:
    pickle.dump(preds_dict,f)

with open('preds_t_dict','wb') as f:
    pickle.dump(preds_t_dict,f)
...
pred_t = np.mean(np.array([preds_t_dict[str(f)] for f in range(10)]),axis=0)
...
pred_t_geo = gmean(np.array([preds_t_dict[str(f)] for f in range(10)]),axis=0)
th_t = fit_test(pred_t_geo,lb_prob)
th_t[th_t<0.1] = 0.1
print('Thresholds: ',th_t)
print('Fractions: ',(pred_t_geo > th_t).mean(axis=0))
print('Fractions (th = 0.5): ',(pred_t_geo > 0.5).mean(axis=0))

save_pred(pred_t,th_t,'protein_classification_f_geo.csv')

save_pred(pred_t,th,'protein_classification_v_geo.csv')
save_pred(pred_t,0.5,'protein_classification_05_geo.csv')

pred_t_max = np.max(np.array([preds_t_dict[str(f)] for f in range(10)]),axis=0)
th_t = fit_test(pred_t_max,lb_prob)
th_t[th_t<0.1] = 0.1
print('Thresholds: ',th_t)
print('Fractions: ',(pred_t_max > th_t).mean(axis=0))
print('Fractions (th = 0.5): ',(pred_t_max > 0.5).mean(axis=0))

save_pred(pred_t,th_t,'protein_classification_f_max.csv')

save_pred(pred_t,th,'protein_classification_v_max.csv')
save_pred(pred_t,0.5,'protein_classification_05_max.csv')



tr_n, val_n = get_fold(0,folds)

train_df = pd.read_csv('Christof/assets/train.csv')
y_f = [train_df['']]
...
ax[0].plot(list(range(8)),learner.sched.val_losses, label='Validation loss')
#ax[0].plot(list(range(8)),learner.sched.losses, label='Training loss')
ax[0].set_xlabel('Epoch')
ax[0].set_ylabel('Loss')
ax[0].legend(loc='upper right')
ax[1].plot(list(range(8)),learner.sched.rec_metrics)
