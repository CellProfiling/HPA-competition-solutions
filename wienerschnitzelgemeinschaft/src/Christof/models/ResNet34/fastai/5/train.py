...
MODEL_PATH = 'Christof/models/ResNet34/fastai/5/'
...
from fastai_callbacks import TensorboardLogger
tb_logger = TensorboardLogger(learner.model, md, '1', path=MODEL_PATH + 'logs/')
# fit 1 epoch
lr = 2e-2
learner.fit(lr,1, callbacks = [tb_logger])

#unfreeze and set differential lr
learner.unfreeze()
lrs=np.array([lr/10,lr/3,lr])

learner.fit(lrs/4,4,cycle_len=2,use_clr=(10,20),callbacks = [tb_logger])
...
preds_t,y_t = learner.predict(is_test=True)
...
save_pred(pred_t,th_t,MODEL_PATH + 'protein_classification_f.csv')

save_pred(pred_t,th,MODEL_PATH + 'protein_classification_v.csv')
save_pred(pred_t,0.5,MODEL_PATH + 'protein_classification_05.csv')

class_list = [8,9,10,15,20,24,27]
for i in class_list:
    th_t[i] = th[i]
save_pred(pred_t,th_t,MODEL_PATH + 'protein_classification_c.csv')