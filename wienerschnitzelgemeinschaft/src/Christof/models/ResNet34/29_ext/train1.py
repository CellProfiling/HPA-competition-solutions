...
MODEL_PATH = 'Christof/models/ResNet34/29_ext/'
exp_suffix = '_1'
...
class data_generator:
    ...
    @staticmethod
    def create_train(dataset_info, batch_size, shape, augument=None, weighted_sample = True,mean_g = 0.05258,std_g = 0.0879):
        assert shape[2] == 3
...
train_generator_ext1 = data_generator.create_train(train_dataset_info_ext1,
                                              batch_size, (SIZE, SIZE, 3), augument=normal_aug_ext, mean_g=0.06798691, std_g=0.10036821)
import random
...
dp = (draw_predict + 2*draw_predict_map32) / 3
pred = dp.copy()
thresholds = np.linspace(0.95, 0.05, 101)
for j in tqdm(range(pred.shape[1])):
    for t in thresholds:
