...
callbacks_list = [lr_schedule,f1_field_metric, checkpoint, checkpoint2,tensorboard]
...
def corrected_augment(image):
    """
    after training bug fix
    :param image:
    :return:
    """
    mean_g = 0.05258
...
for i, name in tqdm(enumerate(submit['Id'])):
    ...
    images = [corrected_augment(image) for _ in range(tta)]
...
submit.to_csv(MODEL_PATH + 'corrected_submission_loss{}_lb_dist_adjusted_8tta.csv'.format(exp_suffix), index=False)
...
submit['Predicted'] = str_predict_label
# np.save('draw_predict_InceptionV3.npy', score_predict)
submit.to_csv(MODEL_PATH + 'corrected_submission_loss{}_lb_dist_adjusted_map32_8tta.csv'.format(exp_suffix), index=False)
