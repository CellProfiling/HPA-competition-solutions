...
checkpoint = ModelCheckpoint(MODEL_PATH + 'model_f{}_focal.h5'.format(fold_id), monitor='val_f1_all', verbose=1,
                             save_best_only=True, mode='max', save_weights_only=True)
tensorboard = TensorBoard(MODEL_PATH + 'logs{}_focal_f1'.format(fold_id) + '/')
...
def focal_loss_d(y_true, y_pred):
    # y_true = K.sigmoid(logits)
    gamma = 2.
    alpha = 1.
...
def focal_f1(y_true,y_pred):#

    return focal_loss_d(y_true,y_pred) + soft_f1_loss(y_true,y_pred)


model.compile(loss=focal_f1,
...
model.save_weights(MODEL_PATH + 'model_f{}_e{}_focal.h5'.format(fold_id,epochs))
model.load_weights(MODEL_PATH + 'model_f{}_e50_focal.h5'.format(fold_id))
