...
# Load dataset info
path_to_train = 'Christof/assets/train_rgby_512/'
...
normal_aug = A.Compose([#A.Rotate((0,30),p=0.75),
                        ...
                        A.Normalize(mean=(0.08069, 0.05258, 0.05487,0.08282), std=(0.1300, 0.0879, 0.1386,0.1272),
                                    max_pixel_value=255.)
                        ])
...
normal_aug_ext = A.Compose([#A.Rotate((0,30),p=0.75),
                        ...
                        A.IAAAffine(translate_percent=10,rotate=45,shear=10, scale=(0.9,1.1)),
                        #A.RandomAffine(degrees=45, translate=(0.1,0.1), shear=10, scale=(0.9,1.1))
                        A.Normalize(mean=(0.1174382,  0.06798691, 0.06592218,0.09499568), std=(0.16392466 ,0.10036821, 0.16703453,0.13726404),
                                    max_pixel_value=255.)
                        ])
...
path_to_train_ext1 = 'Christof/assets/ext_tomomi_rgby/'
fns = os.listdir(path_to_train_ext1)
train_dataset_info_ext1 = []
for name, labels in zip(data_ext1['Id'], data_ext1['Target'].str.split(' ')):
    if name[:-5] + '.png' in fns:
...
def create_model(input_shape, n_out):
    ...
    base_model2 = ResNet18(include_top=False,
                             weights=None,
                             input_shape=(SIZE, SIZE, 4),input_tensor=input_tensor2)

    # transfer weights of deeper layers
    for k, layer in enumerate(base_model2.layers):
        if k > 4:
            base_model2.layers[k].set_weights(base_model.layers[k].get_weights())
...
model.load_weights(MODEL_PATH + 'model_loss_0.h5')
model.fit_generator(
    train_generator,
...
for i, name in tqdm(enumerate(submit['Id'])):
    path = os.path.join('Christof/assets/test_rgby_512/', name)
