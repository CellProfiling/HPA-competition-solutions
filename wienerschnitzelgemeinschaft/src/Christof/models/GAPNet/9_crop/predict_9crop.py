...
draw_predict = np.zeros((6,len(submit['Id']), 28))
for id, prefix in enumerate(['ul','um','ur','mm','bl','br']):
    draw_predict[id] = np.load(MODEL_PATH + f'pred1_{prefix}.npy')


#draw_predict1 = np.mean(draw_predict,axis = 0)
draw_predict1 = draw_predict[0] + 0.8*draw_predict[1] + draw_predict[2] + 1.2*draw_predict[3] +draw_predict[4] + draw_predict[5]
draw_predict1 = draw_predict1 / 6
