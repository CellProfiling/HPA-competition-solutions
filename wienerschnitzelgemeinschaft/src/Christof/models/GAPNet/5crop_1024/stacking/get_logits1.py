...
stats = dict(
    ul_stats=dict(mean=(0.08150311, 0.05296481, 0.05292799), std=(0.11785979, 0.07682588, 0.12809756)),
    ul_stats_ext=dict(mean=(0.11850064, 0.06893709, 0.06541808), std=(0.16187225, 0.09886741, 0.1608865)),
    ur_stats=dict(mean=(0.08165012, 0.0530909, 0.05298166), std=(0.12806622, 0.08622692, 0.13038702)),
    ur_stats_ext=dict(mean=(0.11843426, 0.06886751, 0.06541236), std=(0.16149608, 0.0987589, 0.16087747)),
    mm_stats=dict(mean=(0.09093833, 0.06286007, 0.06817141), std=(0.12350026, 0.08373273, 0.15111048)),
    mm_stats_ext=dict(mean=(0.13891804, 0.08487125, 0.08404315), std=(0.1703862, 0.10835427, 0.18706174)),
    bl_stats=dict(mean=(0.07957268, 0.05282806, 0.05699782), std=(0.11762907, 0.07754794, 0.13799691)),
    bl_stats_ext=dict(mean=(0.11663497, 0.06764909, 0.0661633), std=(0.16031308, 0.09734452, 0.16317564)),
    br_stats=dict(mean=(0.07944266, 0.05218451, 0.05687321), std=(0.1172611, 0.07672443, 0.13864176)),
    br_stats_ext=dict(mean=(0.1166027, 0.0671658, 0.06609571), std=(0.16027017, 0.09698545, 0.16410226)))
...
oof_logits = np.zeros((len(train_dataset_info),256,5))
for i,tile in enumerate(['ul','ur','mm','bl','br']):
    #tile = 'ul'
    path_to_train = f'Christof/assets/train_rgb_1024_9crop/{tile}/'

    model.load_weights(MODEL_PATH + f'model_loss{fold_id}_{tile}.h5')
    logit_model = Model(model.input,model.layers[-3].output)
    val_aug = A.Compose([A.HorizontalFlip(p=0.5),
                          A.Normalize(stats[f'{tile}_stats']['mean'],stats[f'{tile}_stats']['std'],
                                      max_pixel_value=255.)])
    ...
    # validation_generator = data_generator.create_train(train_dataset_info[val_ids],
    #                                                    32, (SIZE, SIZE, 3), augument=val_aug, weighted_sample=False)
    #
    oof_logits_tile = np.zeros((len(train_dataset_info),256))
    #val_id = val_ids[0]
    for val_id in tqdm(val_ids):
        ...
        tta_predicts = logit_model.predict(np.array(images))
        oof_logits_tile[val_id] = np.mean(tta_predicts,axis = 0)
    np.save(MODEL_PATH + f'oof_logits_{fold_id}_{tile}.npy',oof_logits_tile)
    oof_logits[:,:,i] = oof_logits_tile

labels = [item['labels'] for item in train_dataset_info]
fold1_labels = [labels[val_id] for val_id in val_ids]

fold1_labs = np.zeros((6212,28))
for i,item in enumerate(fold1_labels):
    fold1_labs[i][item] = 1
fold1_oof_logits = oof_logits[val_ids]
...
stacker = Model(inp,out)
stacker.compile(optimizer=Adam(0.0005),loss='binary_crossentropy')
from sklearn.model_selection import  train_test_split
X_train, X_val, y_train, y_val = train_test_split(fold1_oof_logits, fold1_labs)

stacker.fit(X_train, y_train, validation_data=(X_val,y_val), batch_size=64, epochs=5)
...
f1_score(y_val, preds>0.2, average='macro')
