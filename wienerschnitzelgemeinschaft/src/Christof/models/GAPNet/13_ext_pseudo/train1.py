...
MODEL_PATH = 'Christof/models/GAPNet/13_ext_pseudo/'
...
test_dataset_info = []
test_data = pd.read_csv('Christof/models/GAPNet/13_ext/submission_loss_1_lb_dist_adjusted_8tta.csv')
path_to_test = 'Christof/assets/test_rgb_512/'
test_data['Predicted'] = test_data['Predicted'].fillna(' ')

test_data = test_data[test_data['Predicted'] != ' ']
...
train_indexes, valid_indexes = next(kf)
train_generator_orig = data_generator.create_train(np.concatenate([train_dataset_info[train_indexes],test_dataset_info]),
...
model.fit_generator(
    train_generator,
    steps_per_epoch=np.ceil(float(2*(len(train_indexes)+len(test_dataset_info))) / float(batch_size)),
