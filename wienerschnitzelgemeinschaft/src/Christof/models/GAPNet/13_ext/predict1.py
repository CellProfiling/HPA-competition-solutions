...
tta_aug = A.Compose([#A.Rotate((0,30),p=0.75),
                        A.RandomRotate90(p=1),
                        A.HorizontalFlip(p=0.5),
                        #A.RandomBrightness(0.05),
                        #A.RandomContrast(0.05),
                        #A.IAAAffine(translate_percent=10,rotate=45,shear=10, scale=(0.9,1.1)),
                        #A.RandomAffine(degrees=45, translate=(0.1,0.1), shear=10, scale=(0.9,1.1))
                        A.Normalize(mean=(0.05908022, 0.04532852, 0.04065233), std=(0.10371015, 0.07984633, 0.10664798),
                                    max_pixel_value=255.)
                        ])
...
for i, name in tqdm(enumerate(submit['Id'])):
    ...
    images = [data_generator.augment(tta_aug, image) for _ in range(tta)]
...
submit.to_csv(MODEL_PATH + 'submission_loss{}_lb_dist_adjusted_8tta_tta_aug.csv'.format(exp_suffix), index=False)
