...
for fold_id in range(5):
    ...
    tta = 8
    model.load_weights(MODEL_PATH + f'model_loss_40_{fold_id}.h5')
    #draw_predict = np.zeros((len(submit['Id']), 28))

    for i, name in tqdm(enumerate(submit['Id'])):
        path = os.path.join('Christof/assets/test_rgb_1024/', name)
...
np.save(MODEL_PATH + f'pred{suffix}.npy', draw_predict)
