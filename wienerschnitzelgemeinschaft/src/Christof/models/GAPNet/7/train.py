...
MODEL_PATH = 'Christof/models/GAPNet/7/'
exp_suffix = '_base'
...
def encoder(backbone):
    ...
    c1 = backbone.get_layer('conv1/relu').get_output_at(0)  # 128
    c2 = backbone.get_layer('pool2_conv').output  # 64
    enc_out = backbone.get_layer('pool3_relu').output  # 32
...
#                      mode="min",
#                      patience=6)
f1_metric = F1Metric(validation_generator2,2*len(valid_indexes)//(2*batch_size),2*batch_size,28) #2 times val because of val_aug
...
sub1 = pd.read_csv('Christof/models/GAPNet/5/submission_loss_base_lb_dist_adjusted_8tta.csv')
f1_sub(best_sub,sub1)