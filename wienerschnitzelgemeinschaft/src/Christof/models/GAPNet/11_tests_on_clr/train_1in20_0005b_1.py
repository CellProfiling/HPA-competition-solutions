...
def create_model(input_shape, n_out):
    ...
    # replace (wrong) bn weights
    w = [np.array([0.,0.,0.],dtype=np.float32),np.array([0.,0.,0.],dtype=np.float32),np.array([1.,1.,1.],dtype=np.float32)]
    base_model.layers[1].set_weights(w)
