...
accuracy_score(y_true, preds_05)

acc = np.mean([accuracy_score(y_true[:,i], preds[:,i]) for i in range(28)])
...
sample_submit.to_csv(MODEL_PATH + 'predictions/' + 'sub_best_ckpt_fold0.csv', index=False)
