...
target_size = 256
target_path = 'assets/train_256_2/'
...
for id in tqdm(ids):
    ...
    rgba_image[:, :, :3] = rgba_image[:, :, :3] / 2
    rgba_image[:, :, :3] = rgba_image[:, :, :3] + rgba_image[:, :, 3:] / 2
    cv2.imwrite(target_path + id + '.png',rgba_image[:, :, :3])
