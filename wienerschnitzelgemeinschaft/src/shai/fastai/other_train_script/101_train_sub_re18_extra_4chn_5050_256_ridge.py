...
LABELS_ext_train = '../input/train_ext_unbalanced.csv'
LABELS_ext_val = '../input/valid_ext_unbalanced.csv'
...
# Data
# =======================================================================================================================
df_train_fld1 = pd.read_csv(LABELS_org)
df_val_fld1 = pd.read_csv(LABELS_ext_val)
df_train_ext = pd.read_csv(LABELS_ext_train)
...
tr_n = df_train_fld1['Id'].values.tolist() + df_train_ext['Id'].values.tolist() + df_train_fld1['Id'].values.tolist()
shuffle(tr_n)

val_n = df_val_fld1['Id'].values.tolist()[:4000]
...
tb_logger = TensorboardLogger(learner.model, md, "res18_256_unbalanced_ext_4chn_4060", metrics_names=["precision", 'recall', 'f1'])
...
#learner.fit(lrs / 4, 4, cycle_len=2, use_clr=(10, 20), cycle_save_name='res18_256_ext_4chn_4060_1', callbacks=[tb_logger])
#learner.fit(lrs / 4, 2, cycle_len=4, use_clr=(10, 20), cycle_save_name='res18_256_ext_4chn_4060_2', callbacks=[tb_logger])
#learner.fit(lrs / 16, 1, cycle_len=8, use_clr=(5, 20), cycle_save_name='res18_256_ext_4chn_4060_3',use_swa=True, swa_start=1, callbacks=[tb_logger])

#=======================================================================================================================
# learner.load('resxt50_512_ext_2_cyc_1')
# learner.fit(lrs/4,1,cycle_len=8,use_clr=(5,20),  cycle_save_name='resxt50_512_ext_4', use_swa=True, swa_start=1, callbacks=[tb_logger])
#learner.load('resxt50_512_ext_4_cyc_0-swa')

# =======================================================================================================================
# Submission
# =======================================================================================================================

# Actually trained on 5050 portion
models = ['res18_256_ext_4chn_5050_3_cyc_0', 'res18_256_ext_4chn_5050_3_cyc_0-swa']
#=======================================================================================================================
# Bagging
########################################################################################################################
...
for imodel in models:
    ...
    clf0  = ridge(y, pred, 0, 1)    #42.10  +0.004
    clf1  = ridge(y, pred, 1, 5)    #4.14   +0.01
    clf2  = ridge(y, pred, 2, 2)    #12.38  +0.02
    clf3  = ridge(y, pred, 3, 5)    #5.54   +0.02
    clf4  = ridge(y, pred, 4, 4)    #6.55 # -0.01/5 -0.02/3
    clf5  = ridge(y, pred, 5, 2)    #9.38   +0.01
    clf6  = ridge(y, pred, 6, 6)    #3.46   +0.02
    clf7  = ridge(y, pred, 7, 2)    #9.62   +0.03
    clf8  = ridge(y, pred, 8, 5)    #0.23   +0.02
    clf9  = ridge(y, pred, 9, 5)    #0.15   +0.02
    clf10 = ridge(y, pred, 10, 5)   #0.15   +0.00
    clf11 = ridge(y, pred, 11, 5)   #3.70   +0.07
    clf12 = ridge(y, pred, 12, 6)   #2.38   +0.01
    clf13 = ridge(y, pred, 13, 6)   #1.94   +0.04
    clf14 = ridge(y, pred, 14, 5)   #4.13   +0.02
    clf15 = ridge(y, pred, 15, 10)   #0.09  +0.8
    clf16 = ridge(y, pred, 16, 5)   #1.92   -0.04/6 -0.10/8 -0.11/3 +0.00/5
    clf17 = ridge(y, pred, 17, 7)   #0.92   -0.05/5 -0.08/15 -0.2/3 -0.09/10 -0.05/6
    clf18 = ridge(y, pred, 18, 10)   #3.20  +0.06
    clf19 = ridge(y, pred, 19, 6)   #5.14   -0.02
    clf20 = ridge(y, pred, 20, 10)   #0.63  +0.14
    clf21 = ridge(y, pred, 21, 2)   #13.59  +0.02
    clf22 = ridge(y, pred, 22, 5)   #2.95   +0.04
    clf23 = ridge(y, pred, 23, 2)   #12.54  +0.02
    clf24 = ridge(y, pred, 24, 6)   #1.12   +0.00
    clf25 = ridge(y, pred, 25, 1)   #26.82  +0.02
    clf26 = ridge(y, pred, 26, 10)   #1.17   -0.10/6 -0.04/8
    clf27 = ridge(y, pred, 27, 10)   #0.06  +0.00

    print('f1-macro_before:',sum(f1_before) / 28)
    print('f1-macro_after:', sum(f1_after) / 28)
