...
# =======================================================================================================================
fold = 0   # specify
archt = 'resxt50_rgby_osh' # specify for output folder creation
SUB_DIR = 'subs/' + archt
...
#TRAIN = '/kaggle_data/human_protein/train_all_png4_scale/' #train images
#TEST = '/kaggle_data/human_protein/test_png4/'  # test images 
...
class Oversampling:
    def __init__(self,path):
        ...
        #set the minimum number of duplicates for each class (heuristic scaling)
        self.multi = [1,1,1,1,1,1,1,1,2,2,2,1,1,1,\
			1,4,2,4,1,1,4,1,1,1,1,1,2,8]
...
s = Oversampling(os.path.join(PATH,LABELS_all))
...
class pdFilesDataset(FilesDataset):
  ...
  #                  cv2.IMREAD_UNCHANGED).astype(np.float32)/255 #(Kevin)
  #      img = cv2.imread(os.path.join(self.path, self.fnames[i] + '.png'), \
  #                  ).astype(np.float32)/255 #(Kevin)
        if self.sz == 512:
            return img
        else:
...
class ConvnetBuilder_custom():
	...
	layers[0].weight = torch.nn.Parameter(torch.cat((w,w[:,:1,:,:]),dim=1)) # kevin
...
tb_logger = TensorboardLogger(learner.model, md, "resxt50_512_fold{}".format(fold), metrics_names=["precision", 'recall', 'f1'])

lr = 2e-4#1e-3  #(Kevin)


learner.fit(lr,1,cycle_save_name='resxt50_512_osh_4chn_5050_0') #(Kevin)

lrs = np.array([lr / 10, lr / 3, lr])
learner.unfreeze()

'''
learner.fit(lrs / 4, 4, cycle_len=2, use_clr=(10, 20), cycle_save_name='resxt50_512_ext_4chn_5050_1', callbacks=[tb_logger])
learner.fit(lrs / 4, 2, cycle_len=4, use_clr=(10, 20), cycle_save_name='resxt50_512_ext_4chn_5050_2', callbacks=[tb_logger])
learner.fit(lrs / 16, 1, cycle_len=8, use_clr=(5, 20), cycle_save_name='resxt50_512_ext_4chn_5050_3', callbacks=[tb_logger])
'''

#(Kevin 2019_0101)
learner.fit(lrs , 2, cycle_len=1, cycle_mult=2, use_clr=(20,10), cycle_save_name='resxt50_512_osh_4chn_5050_1', callbacks=[tb_logger])
learner.fit(lrs / 4, 1, cycle_len=4, use_clr=(20, 10), cycle_save_name='resxt50_512_osh_4chn_5050_2', callbacks=[tb_logger])
learner.fit(lrs / 16, 1, cycle_len=4, use_clr=(10, 20), cycle_save_name='resxt50_512_osh_4chn_5050_3', callbacks=[tb_logger])
learner.fit(lrs / 64, 1, cycle_len=1, use_clr=(10, 20), cycle_save_name='resxt50_512_osh_4chn_5050_4', callbacks=[tb_logger])
learner.fit(lrs / 256, 1, cycle_len=1, use_clr=(10, 20), cycle_save_name='resxt50_512_ovlog01_4chn_5050_5', callbacks=[tb_logger])
learner.save('resxt50_512_osh_4chn_fld{}'.format(fold))


#print('loading trained coefficients')
#learner.load('resnext50_rgb_aug005_a4')
#print('start prediction on validation')
#=======================================================================================================================
...
preds.sort(axis=-1)  #(Kevin)
pred_sm = preds[:,:,8:].mean(axis=-1) # average max 8 out of tta16  #(Kevin)
...
# for Kevin ensembling
val_names = np.array(learner.data.val_ds.fnames)
val_save_pkl = np.column_stack((val_names, pred_sm))

val_fileName = SUB_DIR + '/' + 'oof_sm_{}.pkl'.format(fold)
val_fileObject = open(val_fileName, 'wb')
pkl.dump(val_save_pkl, val_fileObject)
val_fileObject.close()
...
preds_t.sort(axis=-1)  #(Kevin)
pred_t_sm = preds_t[:,:,8:].mean(axis=-1) # average max 8 out of tta16  #(Kevin)
...
fileName = SUB_DIR + '/' + 'pred_t_sm_{}.pkl'.format(fold) # for Kevin's own ensembling
fileObject = open(fileName, 'wb')
pkl.dump(save_pkl, fileObject)
fileObject.close()
