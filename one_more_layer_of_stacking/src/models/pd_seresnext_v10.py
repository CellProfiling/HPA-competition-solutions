...
def pd_seresnext_v10():
    import torch

    model_name = 'pd_seresnext_v10'
    ...
    classes_thresholds, classes_scores = twenty_kfold_threshold(y, y_pred)
    n_classes = len(name_label_dict)
    yp = y_pred.copy()
    for i in range(n_classes):
    ...
    xtra = [RandTransform(squish, {})]
    tfms = get_transforms(do_flip=True, flip_vert=True, max_rotate=180.0,
                          max_zoom=1.2, max_lighting=0.2, max_warp=0.02,
                          p_affine=0.9, p_lighting=0.65, xtra_tfms=xtra)
    data = ImageDataBunch.create(train_ds, valid_ds, test_ds, path=data_dir, device=device,
                                 size=512, bs=24, ds_tfms=tfms, padding_mode='zeros')
    data.normalize(model_stats)
    ...
    learner.model = nn.DataParallel(learner.model)
    learner = learner.load('pd_seresnext_v10_0.6822.pnt')
    ...
    lr = 1e-4
    learner.fit_one_cycle(16, max_lr=get_lrs(lr), div_factor=50)
    ...
    learner.model = nn.DataParallel(learner.model)
    learner = learner.load('pd_seresnext_v10_0.6799.pnt')
