...
def pd_seresnext_v7():
    ...
    def get_lrs(lr):
        return np.logspace(np.log(lr/16), np.log(lr), len(layers), base=np.e)
    ...
    @dataclass
    class Mortidomentation(Callback):
        ...
        def on_batch_begin(self, last_input, last_target, train, **kwargs):
            #global test_shit
            #test_shit = (last_input, last_target)
            #raise Error()

            # 0 - image, 1 - channel
            if train and np.random.random_sample() < self.p:
                last_input[0, 1] = -2.0357
                last_target[0] = 0
    ...
    learner.freeze()
    lr = 1e-3
    learner.fit(1, get_lrs(lr), callbacks=Mortidomentation())
    ...
    learner.unfreeze()
    lr = 1e-3
    learner.fit_one_cycle(8, max_lr=get_lrs(lr), callbacks=[Mortidomentation(0.5)])
    ...
    learner = learner.load('pd_seresnext_v7_0.6765.pnt')
    learner.loss_fn = FocalLoss(5)
    ...
    c_t = np.array(classes_thresholds).copy()
    c_t *= 0.48
    ...
    submit.to_csv('../submits/' + model_name + '_dup.csv',index=False)
    submit.head()
