...
def pd_seresnext_v14():
    import torch

    model_name = 'pd_seresnext_v14'
    ...
    mean_stat = tf_img.__dict__['mean']
    mean_stat.append(np.mean(mean_stat))

    std_stat = tf_img.__dict__['std']
    std_stat.append(np.mean(std_stat))


    # In[6]:


    model_stats = (mean_stat, std_stat)
    model_stats
    ...
    xtra = [f for f in os.listdir('../input/train_rgby/') if '_classes_' in f]
    gclasses = set(list(range(28))) - set([0,25])

    for x in xtra:
        classes = set(map(int, x.split('_classes_')[1].split('.')[0].split('_')))

        if len(gclasses & classes) == 0:
            continue

        ids.append(x.split('.')[0])
        labels.append(' '.join(map(str, classes)))
    ...
    if need_to_prepare_extra:
        ...
        xtra_data.to_csv(data_dir + 'xtra_train_rgby.csv', index=False)
        xtra_data.head(n=3)
    ...
    xtra_data = pd.read_csv(data_dir + 'xtra_train_rgby.csv')
    ...
    def open_rgby(fn:PathOrStr)->Image:
        x = PIL.Image.open(fn)
    ...
    train_files = train.Id.apply(lambda s: '../input/' + 'train_rgby/'+s+'.png')
    ...
    valid_files = valid.Id.apply(lambda s: '../input/' + 'train_rgby/'+s+'.png')
    ...
    test_files = test.Id.apply(lambda s: '../input/' + 'test_rgby/'+s+'.png')
    ...
    data = ImageDataBunch.create(train_ds, valid_ds, test_ds, path=data_dir, device=device,
                                 size=512, bs=20, ds_tfms=tfms, padding_mode='zeros')
    ...
    train_ds.image_opener = open_rgby
    valid_ds.image_opener = open_rgby
    test_ds.image_opener = open_rgby
    ...
    def get_lrs(lr, base=16):
        return np.logspace(np.log(lr/base), np.log(lr), 10, base=np.e)
    ...
    tfms = get_transforms(do_flip=True, flip_vert=True, max_rotate=180.0,
                          ...
                          p_affine=0.9, p_lighting=0.72, xtra_tfms=xtra)
    data = ImageDataBunch.create(train_ds, valid_ds, test_ds, path=data_dir, device=device,
                                 size=512, bs=22, ds_tfms=tfms, padding_mode='zeros')
    data.normalize(model_stats)
    ...
    np.save(results_dir+model_name+'_test_1.npy', preds.copy())
    ...
    tfms = get_transforms(do_flip=True, flip_vert=True, max_rotate=180.0,
                          ...
                          p_affine=0.9, p_lighting=0.72, xtra_tfms=xtra)
    data = ImageDataBunch.create(train_ds, valid_ds, test_ds, path=data_dir, device=device,
                                 size=512, bs=25, ds_tfms=tfms, padding_mode='zeros')
    data.normalize(model_stats)
    ...
    learner.model[-1][3].p = 0.6
    learner.model = nn.DataParallel(learner.model)

    learner = learner.load('pd_seresnext_v14_0.7157.pnt')
    ...
    lr = 2e-5
    learner.fit_one_cycle(16, max_lr=get_lrs(lr, base=20))
    ...
    np.save(results_dir+model_name+'_y.npy', y)
    np.save(results_dir+model_name+'_ids.npy', valid.Id.values)
    np.save(results_dir+model_name+'_holdout_2.npy', avg_preds)
    ...
    np.save(results_dir+model_name+'_test_2.npy', preds.copy())
    ...
    avg_preds_cum = np.load(results_dir+model_name+'_holdout_1.npy')
    avg_preds_cum = np.mean([avg_preds, avg_preds_cum], axis=0)
    ...
    c_t = np.array(classes_thresholds).copy() * 0.48


    # In[99]:


    preds_cum = np.load(results_dir+model_name+'_test.npy')
    preds_cum = np.mean([preds, preds_cum], axis=0)


    # In[103]:


    preds_cum[869, 0], preds[869, 0]


    # In[104]:


    ppreds = np.zeros(preds.shape)
    ...
    submit.to_csv('' + model_name + '_sa.csv',index=False)
    submit.head(n=3)
    ...
    duplicates = pd.read_csv('../input/leak_v4.csv')
    tests = duplicates['Unnamed: 0'].tolist()
    classes = duplicates['Site Parsed Labels'].tolist()

    for i, test in enumerate(tests):
        submit.loc[submit['Id'] == test, 'Predicted'] = classes[i]


    # In[107]:


    submit[submit['Predicted'].apply(len) == 0].shape[0]


    # In[108]:


    submit.to_csv('../submits/' + model_name + '_sa_dup.csv',index=False)
    submit.head()
