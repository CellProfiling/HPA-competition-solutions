...
def pd_bninception_v4():
    import torch

    model_name = 'pd_bninception_v4'
    ...
    xtra_data = pd.read_csv(data_dir + 'xtra_train_full_size.csv')
    ...
    train_files = train.Id.apply(lambda s: '../input/' + 'train_full_size/'+s+'.png')
    ...
    valid_files = valid.Id.apply(lambda s: '../input/' + 'train_full_size/'+s+'.png')
    ...
    test_files = test.Id.apply(lambda s: '../input/' + 'test_full_size/'+s+'.png')
    ...
    data = ImageDataBunch.create(train_ds, valid_ds, test_ds, path=data_dir, device=device,
                                 size=768, bs=8, ds_tfms=tfms, padding_mode='zeros')
    ...
    learner.freeze()
    lr = 2e-3
    learner.fit_one_cycle(4, get_lrs(lr))


    # In[24]:


    learner.save('bninception-768-stage-1')
    ...
    learner.load('bninception-768-stage-1')
    learner.unfreeze()
    lr = 2e-3
    ...
    learner.save('bninception-768-stage-2')
    ...
    learner = learner.load('pd_bninception_v4_0.6158.pnt')
    ...
    tfms = get_transforms(do_flip=True, flip_vert=True, max_rotate=180.0,
                          ...
                          p_affine=0.9, p_lighting=0.7, xtra_tfms=xtra)
    data = ImageDataBunch.create(train_ds, valid_ds, test_ds, path=data_dir, device=device,
                                 size=768, bs=12, ds_tfms=tfms, padding_mode='zeros')
    data.normalize(model_stats)
    ...
    learner = learner.load(model_name+'_0.6597.pnt')
