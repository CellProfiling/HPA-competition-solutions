...
def pd_seresnext_ovr_v3():
    import torch

    model_name = 'pd_seresnext_ovr_v3'
    ...
    plt.xticks(range(28), list(name_label_dict.values()), rotation='vertical')
    plt.bar(range(28), classes)


    # In[17]:


    f_scores = np.array([0.86347, 0.82206, 0.82360, 0.74144, 0.76625, 0.71534, 0.53757,
                0.79614, 0.86957, 0.60000, 0.66667, 0.73934, 0.68936, 0.68508,
                0.80576, 0.47059, 0.53548, 0.45070, 0.50420, 0.61877, 0.50000,
                0.69599, 0.57576, 0.82120, 0.80899, 0.68537, 0.45946, 0.60870])

    labels = np.array(list(name_label_dict.values()))[np.argsort(f_scores)]
    #f_scores = f_scores[np.argsort(f_scores)]

    plt.figure(figsize=(12, 8))
    plt.xticks(range(28), labels, rotation='vertical')
    plt.bar(range(28), classes[np.argsort(f_scores)])
    ...
    bad_labels = np.arange(28)[np.argsort(f_scores)][:10]
    map_dict = {bad_labels[i]:str(i) for i in range(bad_labels.shape[0])}

    def filter_labels(x):
        labels = list(map(int, x.split(' ')))
        target = []

        for b_l in bad_labels:
            if b_l in labels:
                target.append(map_dict[b_l])

        return ' '.join(target)

    filter_labels('2 25 0 6 17 22')
    ...
    n_l_d = {i:v for i, v in enumerate([name_label_dict[k] for k in name_label_dict if k in bad_labels])}


    # In[21]:


    train['Target'] = train['Target'].apply(filter_labels)
    valid['Target'] = valid['Target'].apply(filter_labels)
    name_label_dict = n_l_d
    name_label_dict
    ...
    train_labels = train.Target.astype(str).apply(lambda s : [name_label_dict[int(q)] for q in s.split(' ') if q!=''])
    ...
    valid_labels = valid.Target.astype(str).apply(lambda s : [name_label_dict[int(q)] for q in s.split(' ') if q!=''])
    ...
    test_labels = test.Predicted.astype(str).apply(lambda s : [name_label_dict[int(q)] for q in s.split(' ') if q!=''])
    ...
    def get_lrs(lr, base=16):
        return np.logspace(np.log(lr/base), np.log(lr), 6, base=np.e)
    ...
    def imagenet_body(pretrained=True):
    ...
    learner = create_cnn(data, arch=imagenet_body, cut=-2,
                         custom_head=create_head(4096, 28, ps=0.4))
    learner.loss = FocalLoss(5)


    # In[28]:


    learner.load('pd_seresnext_v6_0.7096.pnt')
    ...
    learner = create_cnn(data, arch=body, cut=-1,
    ...
    learner = learner.load('ovr-2-stage-1')
    learner.unfreeze()
    lr = 5e-5
    ...
    # 0.51
    learner = learner.load('ovr-2-stage-2')
