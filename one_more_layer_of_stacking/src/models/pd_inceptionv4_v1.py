...
def pd_inceptionv4_v1():
    model_name = 'pd_inceptionv4_v1'
    ...
    learner = create_cnn(data, arch=body, cut=-2,
                         custom_head = create_head(3072, len(data.classes), ps=0.3)) #
    learner.loss_fn = FocalLoss()

    learner.split([learner.model[0][0][:3],learner.model[0][0][3:5],learner.model[0][0][5:11],
                   learner.model[0][0][11:18],learner.model[0][0][18:],learner.model[1]
                  ])
    ...
    learner.freeze()
    lr = 1e-3
    learner.fit(4, [lr,lr/2,lr/4,lr/8,lr/16,lr])


    # In[21]:


    learner.save('inc-stage-1')


    # In[22]:


    learner.load('inc-stage-1')
    ...
    learner.unfreeze()
    lr = 5e-4
    learner.fit_one_cycle(8, max_lr=[lr/16,lr/12,lr/8,lr/4,lr/2,lr])
    ...
    learner = learner.load('pd_inceptionv4_v1_0.6728.pnt')
    ...
    lr = 5e-5
    learner.fit_one_cycle(16, max_lr=[lr/16,lr/16,lr/8,lr/4,lr/2,lr])
    ...
    classes_thresholds, classes_scores = kfold_threshold(y, avg_preds)
    n_classes = len(name_label_dict)
    yp = avg_preds.copy()
    for i in range(n_classes):
    ...
    preds = learner.TTA(ds_type=DatasetType.Test)

    ppreds = preds[0].cpu().numpy()
    ...
    for i in range(28):
        ppreds[:,i] = ppreds[:,i] >= classes_thresholds[i]
    ...
    submit.to_csv('../submits/' + model_name + '.csv',index=False)
    submit.head(n=3)
    ...
    results_dir = '../results/'
    np.save(results_dir+model_name+'_holdout.npy', y_pred)
    np.save(results_dir+model_name+'_test.npy', preds[0].cpu().numpy())
