...
def pd_inceptionv4_v2():
    model_name = 'pd_inceptionv4_v2'
    ...
    data = ImageDataBunch.create(train_ds, valid_ds, test_ds, path=data_dir, device=device,
                                 size=512, bs=16, ds_tfms=tfms, padding_mode='zeros')
    ...
    learner = learner.load('inc-stage-1')
    learner.model[1][3].p = 0.4
    ...
    learner.save('inc-stage-2')


    # In[23]:


    learner = learner.load('inc-stage-2')
    learner.unfreeze()
    ...
    learner.save('pd_inceptionv4_v2-stage-2')
    ...
    xtra = [RandTransform(squish, {})]
    tfms = get_transforms(do_flip=True, flip_vert=True, max_rotate=180.0,
                          max_zoom=1.25, max_lighting=0.25, max_warp=0.07,
                          p_affine=0.9, p_lighting=0.65, xtra_tfms=xtra)
    data = ImageDataBunch.create(train_ds, valid_ds, test_ds, path=data_dir, device=device,
                                 size=512, bs=16, ds_tfms=tfms, padding_mode='zeros')
    data.normalize(model_stats)
    ...
    y_pred_solo, avg_preds1, y = learner.TTA(ds_type=DatasetType.Test, beta=None)
    y = y.cpu().numpy().copy()
    _, avg_preds2, _ = learner.TTA(ds_type=DatasetType.Test, beta=None)
    _, avg_preds3, _ = learner.TTA(ds_type=DatasetType.Test, beta=None)
    _, avg_preds4, _ = learner.TTA(ds_type=DatasetType.Test, beta=None)


    # In[47]:


    preds = y_pred_solo.cpu().numpy().copy()*0.4+torch.stack([avg_preds1,avg_preds2,avg_preds3,avg_preds4]).mean(0).cpu().numpy().copy()*0.6
