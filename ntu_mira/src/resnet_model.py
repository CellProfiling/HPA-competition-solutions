...
def resnet_backbone_dropout(image, num_blocks, group_func, block_func, num_class=config.NUM_CLASS):
    with argscope(Conv2D, use_bias=False,
                  kernel_initializer=tf.variance_scaling_initializer(scale=2.0, mode='fan_out')):        # Note that this pads the image by [2, 3] instead of [3, 2].
        # Similar things happen in later stride=2 layers as well.
        with backbone_scope(freeze=config.FREEZE, freeze_bn=False if not config.FREEZE_BN else True):
            l = Conv2D('conv0', image, 64, 7, strides=2, activation=BNReLU)
            l = MaxPooling('pool0', l, pool_size=3, strides=2, padding='SAME')
            l = group_func('group0', l, block_func, 64, num_blocks[0], 1)
            l = group_func('group1', l, block_func, 128, num_blocks[1], 2)
            ...
            l = group_func('group3', l, block_func, 512, num_blocks[3], 2)
            l = GlobalAvgPooling('gap', l) 
        with backbone_scope(freeze=False, freeze_bn=False if not config.FREEZE_BN else True):
            if config.DROPOUT:
                #l = tf.contrib.layers.flatten(l)
                l = FullyConnected('linear_0', l, 1024, activation=tf.nn.relu,
                                    kernel_initializer=tf.random_normal_initializer(stddev=0.01))
                l =  Dropout(l) # ratio to keep
            logits = FullyConnected('linear_1', l, num_class,
                                kernel_initializer=tf.random_normal_initializer(stddev=0.01))
    return logits